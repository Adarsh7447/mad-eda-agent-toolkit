create table public.agents_duplicate_confidence_table (
  id text not null,
  list_name text null,
  ...
  constraint agents_duplicate_confidence_table_pkey primary key (id)
) TABLESPACE pg_default;

create index IF not exists agents_duplicate_confidence_table_deduplication_key_idx on public.agents_duplicate_confidence_table using btree (deduplication_key) TABLESPACE pg_default;

create index IF not exists agents_duplicate_confidence_table_normalized_full_name_idx on public.agents_duplicate_confidence_table using btree (normalized_full_name) TABLESPACE pg_default;

create index IF not exists agents_duplicate_confidence_table_normalized_email_idx on public.agents_duplicate_confidence_table using btree (normalized_email) TABLESPACE pg_default;

create index IF not exists agents_duplicate_confidence_table_normalized_phone_idx on public.agents_duplicate_confidence_table using btree (normalized_phone) TABLESPACE pg_default;