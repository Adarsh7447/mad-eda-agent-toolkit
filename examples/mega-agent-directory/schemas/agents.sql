create table public.agents (
  ...
  aryan_medium text null,
  zillow_pro_or_not text null,
  "Zillow_Review" real null,
  "Zillow_review_count" smallint null,
  id_text text GENERATED ALWAYS as ((id)::text) STORED null,
  constraint agents_pkey primary key (id)
) TABLESPACE pg_default;

create index IF not exists idx_agents_dedup on public.agents using btree (deduplication_key) TABLESPACE pg_default;

create index IF not exists idx_agents_name on public.agents using btree (normalized_full_name) TABLESPACE pg_default;

create index IF not exists idx_agents_id_text on public.agents using btree (id_text) TABLESPACE pg_default
where
  (team_id is not null);